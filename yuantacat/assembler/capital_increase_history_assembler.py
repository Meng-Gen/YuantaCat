...
from yuantacat.dao.capital_increase_history_dao import CapitalIncreaseHistoryDao

import lxml.html

class CapitalIncreaseHistoryAssembler():
    def __init__(self):
    ...
    def assemble(self, param):
        ...
        return CapitalIncreaseHistoryDao(column_name_list, row_list, stock_symbol, 'Y')
    ...
    def __assemble_column_name_list(self, relative_html_object):
        ...
        td_texts = tr_tags[1].xpath('./td/text()')
    ...
    def __assemble_row_list(self, relative_html_object):
        # skip the first row of header
        # skip the second row of column name list
        # skip the last two rows of chart
        tr_tags = relative_html_object.xpath('./tr')
        assert len(tr_tags) > 2, 'invalid tr_tags'

        return [self.__assemble_row(tr_tag) for tr_tag in tr_tags[2:-2]]
